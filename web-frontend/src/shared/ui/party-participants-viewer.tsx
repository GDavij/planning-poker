...
import { useListParticipantsOfMatch } from "../hooks/integrations/api/matches/use-list-participants-of-match.integration";
...
export function PartyParticipantsViewer() {
  ...
  const { participants, isFetching } = useListParticipantsOfMatch(matchId);

  const { registerEndpointFor } = useSignalRContext();
  const { voteOrReplace } = useParticipants();
  const { currentShowingStory } = useMatchStore();

  useEffect(() => {
    registerEndpointFor(SignalRHooks.OnParticipantVote, (vote) => {
