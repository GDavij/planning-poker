    ...
    public async Task Handle(long matchId, long storyId)
    {
                                             ...
                                             s.MatchId == matchId))
        {
                                                                                 ...
                                                                                 .Where(sp => sp.StoryId == storyId).AsAsyncEnumerable())
            {
                ...
                await _hubContext.Clients.Group(matchId.ToString()).SendAsync("ParticipantVoteForStoryIs", new { StoryId = storyPoint.StoryId, AccountId = storyPoint.AccountId, Points = storyPoint.Points });
            }
            return;
        }
